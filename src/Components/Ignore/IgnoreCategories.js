...
import { searchIgnoreCategories, CategoriesDDLQuery, CVProcessUnknownCategory } from '../Queries/queries';
import { cloneDeep } from "apollo-utilities";
import { forEach } from "async";

export default class IgnoreCategoriesList extends React.Component {
    ...
    async populateList() {
        ...
        try {
            // console.log('result1');
            result = await execGql('query', searchIgnoreCategories, this.setSearchParams())
            // console.log(result);
        }
        catch (err) {
            errors = err.errorsGql;
        ...
        if (!result) {
            ...
            const dataList = [];
            for (let key in result.data.searchIgnoreCategories) {
                dataList[key] = {
                    attribute: result.data.searchIgnoreCategories[key].attribute,
                    count: result.data.searchIgnoreCategories[key].count,
                    inferance: result.data.searchIgnoreCategories[key].attribute,
                    categories: "Ignore",
                    ...
                    <div className="ui one column grid">
                        <div className="three column row">
                            ...
                            <div className="fourteen wide computer fourteen wide tablet fourteen wide mobile column">
                                <h1 id="title_header"> Ignore Attributes </h1>
